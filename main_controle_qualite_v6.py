...
def update_image():
    ...
    while var_EPI!= 1 and len(liste_messages) == 0:
        ...
        canvas.update()
    if len(liste_messages) != 0 and liste_messages[0][0] == "1":
        canvas.itemconfigure(point, state='hidden')
        canvas.itemconfig(canvas_image,image=image_avant)
        root.update()
    elif len(liste_messages) != 0 and liste_messages[0][0] == "2":
        canvas.itemconfigure(point, state='hidden')
...
def conforme():
    global liste_messages
    client.publish("ECAM_qualite", liste_messages[0] + ";" + get_time() + ";" + "conforme")
    print("Publié : "+ liste_messages[0] + ";" + get_time() + ";" + "conforme")
    del liste_messages[0]
...
def retouche():
    ...
    client.publish("ECAM_qualite", liste_messages[0] + ";" + get_time() + ";" + "retouche")
    print("Publié : "+ liste_messages[0] + ";" + get_time() + ";" + "retouche")
    del liste_messages[0]
    canvas.itemconfig(canvas_image,image=image_portiere_validee)
    canvas.update()
    time.sleep(2)
...
def rebut():
    ...
    button_conforme.config(text = "Conforme", command = conforme, bg = "#13B94D")
    button_non_conforme.config(text = "Non conforme", command = non_conforme)
    client.publish("ECAM_qualite", liste_messages[0] + ";" + get_time() + ";" + "rebut")
    print("Publié : "+ liste_messages[0] + ";" + get_time() + ";" + "rebut")
    del liste_messages[0]
    canvas.itemconfig(canvas_image,image=image_portiere_validee)
    canvas.update()
    time.sleep(2)
